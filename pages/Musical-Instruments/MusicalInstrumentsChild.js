...
import MusicalInstrumentsDetails from "./MusicalInstrumentsDetails";
import ShimmerCardDetails from "../../components/Card/ShimmerCardDetails";
const MusicalInstrumentsChild = () => {
  const {
    fetchedData,
    title,
    nid,
        ...
        <div className="App">
          {location && !location?.pathname?.includes("title") && (
            <Container className="archive-header">
              <Row>
                <Col>
                  {location.pathname.includes("avanaddha") && (
                    <h1>Avanadha Vadya</h1>
                  )}
                  {location.pathname.includes("ghan") && <h1>Ghan Vadya</h1>}
                  {location.pathname.includes("sushir") && (
                    <h1>Sushir Vadya</h1>
                  )}
                  {location.pathname.includes("tat") && <h1> Tat Vadya</h1>}
                  <img
                    src={Divider}
                    alt="Caption Divider"
                    style={{
          ...
          {location && location?.pathname?.includes("title") ? (
            fetchedData && fetchedData.length > 0 ? (
              fetchedData
                ...
                .map((x) => {
                  return (
                    <MusicalInstrumentsDetails key={x.nid} detailData={x} />
                  );
                })
            ) : (
              <ShimmerCardDetails />
...
export default MusicalInstrumentsChild;
