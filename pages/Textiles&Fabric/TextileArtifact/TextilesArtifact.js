...
import TextilesArtifactDetails from "./TextilesArtifactDetails";
import ShimmerCardDetails from "../../../components/Card/ShimmerCardDetails";
const TextilesArtifact = () => {
    ...
    { !response.ok?(<ErrorPage response={response} />):( <div className="App">
        ...
        {location && location?.pathname?.includes("title") ? (
          fetchedData && fetchedData.length > 0 ?(fetchedData
            .filter((x) => x.nid == nid)
            .map((x) => {
              return <TextilesArtifactDetails key={x.nid} detailData={x}  />;
            })):(<ShimmerCardDetails/>)
        ) : (
          <Container ><Row>
...
export default TextilesArtifact;
