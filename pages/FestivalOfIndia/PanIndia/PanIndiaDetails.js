...
function PanIndiaDetails({ detailData }) {
  ...
  const panindiaArray =detailData.field_pan_india_marker && detailData.field_pan_india_marker
    .replaceAll("}, {", "}*{")
    .split("*")
    .map((data) => {
  ...
  const extractedData =detailData.field_dismarker && detailData.field_dismarker
    .replaceAll("], [", "]*[")
    .split("*")
    .map((data) => {
      return JSON.parse(data);
  ...
  const uniqueCategories = [
    ...new Set( extractedData && extractedData.flatMap((item) => item.field_categories_list)),
  ];
  const parser = new DOMParser();
  const doc = panindiaArray.length>0 && parser.parseFromString(
    panindiaArray[0][0].field_slider_images,
    "text/html"
  );
  const images = doc && [...doc.querySelectorAll("img")].map((img) => ({
    src: img.getAttribute("src"),
    alt: img.getAttribute("alt"),
  }));

  console.log("Extracted Data:", images);
  console.log("Unique Categories:", extractedData);

  const caliNaryModal = (data) => {
    const modalarray = extractedData.length>0 && extractedData.filter((modal) => {
  ...
  return (
    <>
      <Container className="mt-4">
        ...
        <Row>
          ...
          <Col lg={12}>
            <Tabs
              ...
              className="mb-3"
            >
              {panindiaArray.length>0 && panindiaArray.map((item, index) => (
                <Tab
                  eventKey={index.toString()}
                  title={item[0].title}
                  key={index}
                >
                  <div dangerouslySetInnerHTML={{ __html: item[0].body }} />
                </Tab>
              ))}
            </Tabs>
          </Col>
        ...
        <Row>
          {uniqueCategories.length>0 && uniqueCategories.map((data) => {
            if (data === "Cuisines") {
              return (
                <Col key={data}>
...
export default PanIndiaDetails;
