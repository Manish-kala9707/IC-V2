...
const handleSearchbox=()=>{
  ...
  // const filteredItems = fullSearchData.filter(item =>
  //   selectedFilters.length === 0 || selectedFilters.includes(item.field_dc_source
  //   )
  // ).filter(item =>
  ...
  return (
    <div className="App">
      ...
      {location && !location?.pathname?.includes("title") && <Container  className="archive-header">
        <Row>
          <Col>
            <h1>traditions and art forms</h1>
          ...
          <Row>
            <Col md={6} xs={12} className="equal">
              ...
              <div onClick={handleShow}>
               <Filter size={40} className="icon-stroke" strokeWidth={1} />
              </div>

              {/* Search bar start */}
              <div className="search-box">
                <button className="btn-search" onClick={handleSearchbox}>
                 <Search size={30} className="icon-stroke" strokeWidth={1} />
                </button>
...
<Offcanvas show={show} onHide={handleClose} backdrop="true"  scroll="false">
      ...
      {location && !location.pathname.includes("title") && <Pagination2 totalPages={totalPages} baseUrl="/digital-district-repository/traditions-and-art-forms" />}
    </div>
  );
};
export default DDRtraditionsartforms;
